...
const frameMap: Map<string, FrameNode> = new Map();   // screenId → Figma frame
const imageQueue: string[] = [];                        // screens waiting for image data
...
async function phase2_createHotspots() {
  ...
  for (let i = 0; i < meta.screens.length; i++) {
    ...
    if (screen.hasTabBar && meta.tabBar?.tabs) {
...
function createHotspotRect(parent: FrameNode, hotspot: Hotspot): RectangleNode {
...
function createTabHotspotRect(parent: FrameNode, tab: TabBarTab): RectangleNode {
  const rect = figma.createRectangle();
...
function buildReaction(hotspot: Hotspot): Reaction | null {
...
function buildTransitionEffect(type: string): Transition | null {
  switch (type) {
    case 'push':
      return {
        type: 'SLIDE_IN',
...
function sendProgress(phase: string, current: number, total: number, detail?: string) {
