  ...
  return (
    <span
      className={`inline-flex items-center justify-center flex-shrink-0 rounded-full p-1 transition-transform duration-300 ${
        open
          ? 'rotate-180 bg-[#206FF7]/15 text-[#206FF7]'
          : 'bg-[#206FF7]/10 text-[#206FF7]'
      }`}
      aria-hidden="true"
    >
      <svg
  ...
  return (
    <div
      className={`rounded-2xl border overflow-hidden transition-all duration-300 ${
        isOpen
          ? 'shadow-md border-[#E5E7EB]/80 bg-white'
          : 'shadow-sm border-[#E5E7EB]/60 bg-white hover:shadow-md'
      }`}
    >
      <button
        id={buttonId}
        type="button"
        onClick={onToggle}
        className="flex justify-between items-center w-full px-6 py-5 text-left cursor-pointer group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#206FF7] focus-visible:ring-offset-2 focus-visible:ring-offset-white"
      ...
      >
        <span
          className={`pr-6 font-semibold text-base md:text-lg transition-colors duration-200 ${
            isOpen ? 'text-[#206FF7]' : 'text-[#0A0A0B] group-hover:text-[#206FF7]'
          }`}
        >
          {item.question}
        </span>
      ...
      >
        <div ref={contentRef} className="px-6 pb-6 pt-0 border-t border-[#E5E7EB]/30">
          {isHtml ? (
            <div
              className="pt-4 prose prose-sm md:prose-base max-w-none text-[#6B7280] leading-relaxed"
              style={{ fontFamily: 'var(--font-montserrat)' }}
            ...
            <p
              className="pt-4 text-[#6B7280] text-base md:text-lg leading-relaxed font-light"
              style={{ fontFamily: 'var(--font-montserrat)' }}
            >
              {item.answer}
            </p>
    ...
    <div role="region" aria-label={lang === 'de' ? 'Häufig gestellte Fragen' : lang === 'pl' ? 'Często zadawane pytania' : 'Frequently Asked Questions'}>
      ...
      <div className="mb-6">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-3 rounded-xl border border-[#E5E7EB] bg-white text-[#0A0A0B] placeholder:text-[#9CA3AF] text-sm font-light transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#206FF7] focus:border-[#206FF7]"
            style={{ fontFamily: 'var(--font-montserrat)' }}
            aria-label={t.search}
          />
          {searchRaw && (
            <button
              ...
              className="absolute right-3 top-1/2 -translate-y-1/2 text-[#9CA3AF] hover:text-[#0A0A0B] transition-colors cursor-pointer"
      ...
      >
        ...
        <button
          ...
          className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#206FF7] focus-visible:ring-offset-2 ${
            !activeCategory
              ? 'bg-[#206FF7] text-white shadow-md'
              : 'bg-[#F5F5F7] text-[#6B7280] hover:bg-[#E5E7EB] hover:text-[#0A0A0B]'
          }`}
          style={{ fontFamily: 'var(--font-montserrat)' }}
        >
          {t.all}
      ...
      >
        {filteredItems.length === 0 ? (
          <div className="text-center py-16">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-[#F5F5F7] mb-4">
          ...
          groupedFiltered.map((group) => (
            <div key={group.category} className="mb-10">
              <h2
                className="text-xl md:text-2xl font-bold mb-5 pl-4 border-l-4 border-[#206FF7]"
